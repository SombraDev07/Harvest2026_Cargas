...
function RuleTable({ rule, totalCount, selectedDistrict }: { rule: typeof RULES[0], totalCount?: number, selectedDistrict: string }) {
  ...
  const downloadXLSX = () => {
    const distParam = selectedDistrict !== 'GERAL' ? `&district=${selectedDistrict}` : '';
    const errorTypeParam = `&error_type=${rule.statsKey}`;
    window.open(`${API_BASE_URL}/loads/export?rule_filter=${rule.filter}${distParam}${errorTypeParam}`, "_blank");
  };
    ...
    <div className="glass rounded-3xl border border-white/10 overflow-hidden mb-6 shadow-2xl">
      ...
      >
        ...
        <div className="flex items-center gap-4 pr-5">
          ...
          {data.length > 0 && (
            <button
              onClick={(e) => { e.stopPropagation(); downloadXLSX(); }}
                ...
                <table className="w-full text-left text-[11px] border-collapse sticky-header">
                  <thead className="sticky top-0 z-20 bg-[#0a0a0a]">
                    <tr className="bg-white/[0.02] text-gray-400 uppercase tracking-tighter text-[9px]">
                      ...
                      <th className="px-3 py-4 font-bold border-b border-white/5">OBS / Inconsistência</th>
                      {['desconto', 'rateio_desconto'].includes(rule.statsKey) && <th className="px-3 py-4 font-bold text-center border-b border-white/5">% Quebra</th>}
                        ...
                        <React.Fragment key={group.key}>
                          ...
                          {group.items.map((item, idx) => (
                            <tr key={`${item.id}-${idx}`} className="hover:bg-white/[0.02] transition-colors group">
                              ...
                              <td className="px-3 py-3 text-gray-500 font-bold">{item.doc_number || 'N/A'}</td>
                              ...
                              {['desconto', 'rateio_desconto'].includes(rule.statsKey) && (
                                <td className="px-3 py-3 text-center">
                                  <span className="px-2 py-1 rounded-lg bg-red-500/20 text-red-400 font-black text-[10px]">
                                    {item.error_message.match(/\((.*?)\)/)?.[1] || 'N/A'}
                                  </span>
                                </td>
                              )}
                              <td className="px-3 py-3">
